...
function navigateTo(view) {
    ...
    document.getElementById('personais-view').classList.add('hidden');
    ...
    if (view === 'home') {
        ...
        renderDashboard();
    } else if (view === 'personais') {
        document.getElementById('personais-view').classList.remove('hidden');
        initPersonaisView();
    }
}
...
function renderDashboard() {
    ...
    document.getElementById('tab-4').classList.toggle('hidden', isPersonal);          // Meu Personal (aluno)
    document.getElementById('tab-5').classList.toggle('hidden', !isPersonal);         // Minha Atuação (personal)

    updateThemeToggleButton();

    if (isPersonal) {
        // Personal inicia em Alunos (3), a menos que ainda não tenha atuação cadastrada
        verificarSePersonalTemAtuacao().then(temAtuacao => switchTab(temAtuacao ? 3 : 5));
    } else {
        switchTab(0); // Aluno inicia na aba de Treinos
    }
}

async function verificarSePersonalTemAtuacao() {
    try {
        const response = await fetch(`${URL_BACKEND}/personais/me`, {
            headers: { 'Authorization': `Bearer ${localStorage.getItem('token_academia')}` }
        });
        if (!response.ok) return true;
        const perfil = await response.json();
        return (perfil.academias || []).length > 0;
    } catch {
        return true;
    }
}
...
function switchTab(tab) {
    ...
    const conteudo = document.getElementById(`content-${tab}`);
    ...
    if (tab === 3) loadAlunos();
    if (tab === 4) carregarMeuPersonal();
    if (tab === 5) carregarMinhaAtuacao();
}
