            ...
            html += `
                <div class="bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 p-5 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:border-emerald-500/50 transition-all shadow-xs" data-aluno-id="${aluno.id}" data-aluno-nome="${aluno.nome}">
                    ...
                    <div class="flex flex-wrap sm:flex-nowrap gap-2 w-full sm:w-auto">
                        ...
                        <button onclick="toggleVincularAluno(${aluno.id}, ${aluno.personal_id || 'null'})"
                                class="${aluno.personal_id ? 'bg-emerald-600 hover:bg-emerald-700 dark:bg-emerald-500 dark:hover:bg-emerald-600 text-white dark:text-black' : 'bg-slate-100 dark:bg-zinc-800 hover:bg-emerald-50 dark:hover:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 border border-slate-300 dark:border-zinc-700 hover:border-emerald-500/40'} px-4 py-2 rounded-xl text-xs font-semibold transition-all flex items-center gap-1.5 justify-center shadow-xs cursor-pointer">
                            <i class="fa-solid ${aluno.personal_id ? 'fa-user-minus' : 'fa-user-plus'}"></i>
                            ${aluno.personal_id ? 'Desvincular' : 'Vincular a mim'}
                        </button>
                    </div>
                </div>`;
        });
...
async function toggleVincularAluno(alunoId, personalAtualId) {
    if (!currentUserId) {
        showToast('Não foi possível identificar seu perfil.', true);
        return;
    }

    const vincular = personalAtualId !== currentUserId;
    if (vincular && personalAtualId && personalAtualId !== currentUserId) {
        if (!confirm(`Esse aluno já está vinculado a outro personal (ID ${personalAtualId}). Deseja assumir o vínculo?`)) return;
    }

    try {
        const novoPersonalId = vincular ? currentUserId : null;
        const response = await fetch(`${URL_BACKEND}/alunos/${alunoId}/personal`, {
            method: 'PATCH',
            headers: { 'Authorization': `Bearer ${localStorage.getItem('token_academia')}`, 'Content-Type': 'application/json' },
            body: JSON.stringify({ personal_id: novoPersonalId })
        });
        const data = await response.json();
        showToast(response.ok ? (data.mensagem || 'Vínculo atualizado!') : `Erro: ${data.detail || 'Falha ao atualizar vínculo.'}`, !response.ok);
        if (response.ok) loadAlunos();
    } catch {
