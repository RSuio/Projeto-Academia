...
class Usuario(Base):
    ...
    personal_id = Column(Integer, ForeignKey("usuarios.id"), nullable=True)

    treinos = relationship("Treino", backref="usuario", cascade="all, delete-orphan")
    academias_atuacao = relationship("PersonalAcademia", backref="personal", cascade="all, delete-orphan")
    alunos_vinculados = relationship(
        "Usuario",
        foreign_keys=[personal_id],
        backref=backref("meu_personal", remote_side=[id]),
    )

    def __init__(self, nome, email, senha, ativo=True, admin=False, especialidade=None, bio=None, personal_id=None):
        self.nome = nome
        self.email = email
        self.senha = senha
        self.ativo = ativo
...
class Academia(Base):
    ...
    personais = relationship("PersonalAcademia", backref="academia", cascade="all, delete-orphan")

    def __init__(self, nome, endereco=None, cidade=None, telefone=None):
        self.nome     = nome
...
class PersonalAcademia(Base):
    __tablename__ = "personal_academia"
    ...
    def __init__(self, personal_id, academia_id, dias_semana=None,
                 horario_inicio=None, horario_fim=None, ativo=True):
        self.personal_id    = personal_id
