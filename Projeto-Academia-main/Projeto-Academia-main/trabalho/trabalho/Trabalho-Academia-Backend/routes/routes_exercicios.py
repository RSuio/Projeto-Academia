...
@router.get("/buscar-por-objetivo")
async def buscar_exercicios_por_objetivo(
    objetivo: str,
    session: Session = Depends(pegar_sessao)
):
    exercicios = session.query(ExercicioCatalogo).filter(ExercicioCatalogo.objetivo == objetivo).all()
