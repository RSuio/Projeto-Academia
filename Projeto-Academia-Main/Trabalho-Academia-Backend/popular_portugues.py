...
def popular_catalogo(session):
    ...
    session.commit()
    print(f"[SUCESSO] {novos} exercicios adicionados ao catalogo.")
...
def main(apenas_inserir=False):
    ...
    if not apenas_inserir:
        print("[AVISO] Apagando catalogo existente...")
        apagar_catalogo(session)

    print("[INFO] Populando com exercicios em portugues...")
    popular_catalogo(session)
    session.close()
    print("\n[OK] Pronto! Catalogo atualizado com sucesso.")
