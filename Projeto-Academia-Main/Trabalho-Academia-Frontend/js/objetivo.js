...
async function carregarObjetivo() {
    ...
    try {
        ...
        if (response.status === 404) {
            container.innerHTML = `
                <div class="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 text-center text-zinc-500">
                    <i class="fa-solid fa-bullseye text-4xl mb-3 opacity-30"></i>
                    <p>Nenhum objetivo definido ainda.</p>
                    <p class="text-xs mt-1">Fale com seu Personal Trainer.</p>
                </div>`;
            return;
        }
        ...
        container.innerHTML = `<p class="text-red-400 text-sm">Erro ao carregar objetivo.</p>`;
    }
}
        ...
        <div class="bg-zinc-900 border border-zinc-800 rounded-3xl p-6">
            ...
            <div class="flex justify-between items-start mb-4">
                <div>
                    ...
                    <p class="text-xs text-zinc-500 mt-1">${dataInicio} → ${dataFim}</p>
                </div>
                <span class="text-xs font-medium px-3 py-1 rounded-full border ${badgeCor}">
            ...
            <div class="flex items-end justify-between mb-3">
                <div>
                    <span class="text-4xl font-bold text-white">${feitos}</span>
                    <span class="text-zinc-400 text-lg"> / ${meta}</span>
                    <span class="text-zinc-500 text-sm ml-1">treinos</span>
                </div>
                <span class="text-2xl font-bold ${concluido ? 'text-emerald-400' : 'text-zinc-300'}">${pct}%</span>
            </div>
...
function abrirModalObjetivo(alunoId, nomeAluno) {
    ...
    modal.innerHTML = `
        <div class="bg-zinc-900 rounded-3xl w-full max-w-md p-8 relative border border-zinc-800">
            ...
            <h2 class="text-2xl font-bold text-white mb-1">Definir Objetivo</h2>
            <p class="text-sm text-zinc-400 mb-6">Aluno: <span class="text-emerald-400 font-medium">${nomeAluno}</span></p>

            <div class="space-y-4">

                <div>
                    <label class="text-xs text-zinc-400 mb-1 block">Número de treinos a completar</label>
                    <input id="obj-meta" type="number" min="1" placeholder="Ex: 24"
                           class="w-full bg-zinc-800 border border-zinc-700 rounded-2xl px-4 py-3 focus:outline-none focus:border-emerald-400 text-white">
                </div>

                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <label class="text-xs text-zinc-400 mb-1 block">Data início</label>
                        <input id="obj-inicio" type="date"
                               class="w-full bg-zinc-800 border border-zinc-700 rounded-2xl px-4 py-3 focus:outline-none focus:border-emerald-400 text-white">
                    </div>
                    <div>
                        <label class="text-xs text-zinc-400 mb-1 block">Data fim (prazo)</label>
                ...
                <div>
                    <label class="text-xs text-zinc-400 mb-1 block">Descrição (opcional)</label>
                    <input id="obj-descricao" type="text" placeholder="Ex: Foco em hipertrofia"
                           class="w-full bg-zinc-800 border border-zinc-700 rounded-2xl px-4 py-3 focus:outline-none focus:border-emerald-400 text-white">
                </div>

                <button onclick="salvarObjetivo(${alunoId})"
                        class="w-full bg-emerald-500 hover:bg-emerald-600 text-black font-bold py-4 rounded-2xl transition-colors mt-2">
                    SALVAR OBJETIVO
                </button>
            </div>
        </div>
