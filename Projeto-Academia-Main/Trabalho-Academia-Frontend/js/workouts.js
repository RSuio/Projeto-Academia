...
async function refreshWorkouts() {
    ...
    try {
        ...
        ORDEM_DIAS.forEach(dia => {
            ...
            header.innerHTML = `
                <span class="text-emerald-400 font-bold text-sm uppercase tracking-widest">${dia}</span>
        ...
        if (semDia.length > 0) {
            if (comDia.length > 0) {
                ...
                header.className = "col-span-full flex items-center gap-3 mt-4 mb-1";
                header.innerHTML = `
                    <span class="text-zinc-500 font-bold text-sm uppercase tracking-widest">Sem dia definido</span>
                    <div class="flex-1 h-px bg-zinc-800"></div>`;
                grid.appendChild(header);
            }
...
function criarCardTreino(treino) {
    ...
    const botaoMarcar = currentRole === 'aluno' ? `
        <button
            ...
            class="mt-5 w-full flex items-center justify-center gap-2 bg-zinc-800 hover:bg-emerald-500 hover:text-black text-zinc-300 text-sm font-medium px-4 py-2.5 rounded-2xl transition-colors border border-zinc-700 hover:border-transparent">
    ...
    card.innerHTML = `
        <div class="flex justify-between items-start mb-2">
            ...
            <span class="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-xs px-3 py-1 rounded-full font-medium whitespace-nowrap ml-2">
